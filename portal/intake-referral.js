...
function ensureReferralDialog(){
  ...
  document.querySelector('#referralConfirmForm')?.addEventListener('submit',async event=>{
    ...
    const body={
      intakeId:selectedId,
      consentConfirmed:document.querySelector('#referralConsent').checked,
      participant:{
    ...
    setWorkspaceMessage('Henvisningen er lukket. En separat deltakerinteresse er opprettet etter bekreftet egen interesse.','success');
...
renderDetail=function(){
  ...
  const next=old.cloneNode(true);old.replaceWith(next);next.id='confirmReferral';next.textContent='Bekreft personens egen interesse';
  const copy=next.closest('.n2-section')?.querySelector('.n2-next-copy');
  if(copy)copy.textContent='Dette oppretter en separat deltakerinteresse med personens egne kontaktopplysninger etter bekreftet samtykke. Først derfra kan VÍA vurderes.';

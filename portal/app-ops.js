...
async function workflowErrorCode(data,error){
 if(data?.error)return String(data.error);
 const response=error?.context;
 if(response){
  try{const payload=await (typeof response.clone==='function'?response.clone():response).json();if(payload?.error)return String(payload.error)}catch{}
 }
...
async function enhanceParticipantDetail(){
 ...
 try{
  ...
  else if(p.stage==='SER'||p.stage==='VIDA'){lastEnhancedParticipant=participantId;return}
  ...
  detail.querySelector('#workflowGateCard')?.remove();const box=document.createElement('div');box.id='workflowGateCard';box.className='panel-card';box.style.marginTop='14px';box.innerHTML=`<p class="eyebrow">Neste gate</p><h3>${label}</h3><p>${hint}</p><div class="form-actions"><a class="ghost" href="./owners.html?participant=${encodeURIComponent(participantId)}">Ansvar / eiere</a>${agreementLink}${formKey?`<a class="primary" href="./form-runner.html?key=${encodeURIComponent(formKey)}&participant=${encodeURIComponent(participantId)}">${label}</a>`:`<button id="workflowGateButton" class="primary" type="button">${label}</button>`}</div><p id="workflowGateMessage" class="message" aria-live="polite"></p>`;detail.appendChild(box);lastEnhancedParticipant=participantId;
  const btn=box.querySelector('#workflowGateButton');if(btn)btn.addEventListener('click',async()=>{btn.disabled=true;const msg=box.querySelector('#workflowGateMessage');msg.textContent='Kontrollerer gate…';const {data,error}=await opsClient.functions.invoke('workflow-command',{body:{action,participantId}});const code=await workflowErrorCode(data,error);if(error||code){const map={PARTICIPANT_AGREEMENT_REQUIRED:'SER kan ikke startes: deltakeravtale, deltakerens egen bekreftelse eller staff-review er ikke ferdig. Åpne «Avtale / beredskap».',PILOT_GO_REQUIRED:'SER kan ikke startes: samlet Pilot-GO mangler eller siste Pilot-GO er ikke GO.',SER_REQUIRES_PILOT:'SER kan ikke startes: deltakeren er ikke knyttet til en aktiv pilot.',NAMED_VIDA_OWNER_REQUIRED:'SER kan ikke startes: navngitt VIDA-eier mangler. Åpne «Ansvar / eiere».',GO_CONDITIONS_OPEN:'SER kan ikke startes: minst ett GO-vilkår er fortsatt åpent. Åpne vilkårsoppgaven og send en ny formell GO / NO-GO-vurdering.',SER_REQUIRES_INDIVIDUAL_GO:'SER kan ikke startes: individuell GO mangler eller deltakeren er ikke i riktig beslutningsfase.',FORBIDDEN:'SER kan ikke startes: rollen din har ikke myndighet til denne overgangen.',MFA_REQUIRED:'SER kan ikke startes før Authenticator/tofaktor er bekreftet.',WORKFLOW_COMMAND_FAILED:'SER-kontrollen svarte ikke som forventet. Ingen data ble endret. Prøv igjen eller bruk oppgavekortet for å se gjeldende blocker.'};msg.textContent=map[code]||`SER kan ikke startes ennå (${code}). Ingen data ble endret.`;btn.disabled=false;return}msg.textContent='Overgang registrert.';lastEnhancedParticipant=null;setTimeout(()=>location.reload(),500)})

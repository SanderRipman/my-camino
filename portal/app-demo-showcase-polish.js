...
const DEMO_SHOWCASE_VERSION='2026-09-24a';
...
function polishRoster(){
  ...
  const cards=[...list.querySelectorAll('.uat-roster-card')];
  cards.forEach(card=>{
    const title=card.querySelector('.uat-roster-top b');if(!title)return;
    const code=title.dataset.demoCode||title.textContent.trim();
    const alias=aliasFor(code);if(!alias)return;
    title.dataset.demoCode=code;title.textContent=alias;card.dataset.demoShowcase='1';
    if(!card.querySelector('.demo-tech-code')){
      const small=card.querySelector('small');
      if(small)small.insertAdjacentHTML('beforeend',`<span class="demo-tech-code"> · ${code}</span>`);
    }
  });
  const aliased=[...list.querySelectorAll('.uat-roster-card[data-demo-showcase="1"]')];
  const technical=[...list.querySelectorAll('.uat-roster-card:not([data-demo-showcase="1"])')];
  [...list.querySelectorAll('.demo-tech-divider')].forEach(x=>x.remove());
  const controls=list.querySelector('.uat-roster-controls');
  let anchor=controls;
  aliased.forEach(card=>{(anchor||list).insertAdjacentElement('afterend',card);anchor=card});
  if(technical.length&&aliased.length){
    const divider=document.createElement('p');divider.className='demo-tech-divider';divider.textContent='Øvrige tekniske testforløp';
    anchor.insertAdjacentElement('afterend',divider);anchor=divider;
  }
  technical.forEach(card=>{(anchor||list).insertAdjacentElement('afterend',card);anchor=card});
}
function polishOverview(){
  const chart=document.querySelector('#overviewChart');const card=chart?.closest('.panel-card');if(!card)return;
  const eyebrow=card.querySelector('.card-head .eyebrow'),heading=card.querySelector('.card-head h3');
  if(eyebrow)eyebrow.textContent='Syntetisk utvikling';
  if(heading)heading.textContent='Demo 2027 · Camino Portugués';
  ...
  note.textContent='Fiktive, lagrede målepunkter for 8 demonstrasjonsdeltakere. Grafen viser ekte systemdata fra demo – ikke faktisk pilotresultat.';
}
function polishAnalysis(){
  ...
  if(note)note.innerHTML='<strong>Demo 2027:</strong> Grafene bruker lagrede, syntetiske gruppemålinger fra Camino Portugués-demoforløpet. Ingen reelle deltakerdata inngår.';
}
function ensureStyle(){
  ...
  const s=document.createElement('style');s.id='demo-showcase-style';s.textContent=`
    ...
    .demo-tech-divider{margin:15px 2px 8px;font-size:11px;font-weight:800;letter-spacing:.05em;text-transform:uppercase;color:#7b8588}

...
const NAV_IA_VERSION='2026-09-05c';
const NAV_SNAPSHOT_KEY='aidme:navigation-snapshot:v3';
...
function snapshotBadges(el){
  return [...(el?.querySelectorAll?.('.nav-count')||[])].map(node=>({text:(node.textContent||'').trim(),classes:safeToneClasses(node),label:node.getAttribute('aria-label')||''})).filter(x=>/^\d+$/.test(x.text));
}
function appendBadges(el,badges=[]){
  if(!badges.length)return;
  const host=document.createElement('i');host.className='nav-badges';
  for(const badge of badges){
    const span=document.createElement('span');span.className=['nav-count',...(Array.isArray(badge.classes)?badge.classes:[])].join(' ');
    span.textContent=String(badge.text||'');if(badge.label)span.setAttribute('aria-label',badge.label);host.appendChild(span);
  }
  el.appendChild(host);
}
...
const PAGE_META={
  ...
  guide:{num:'00',label:'Slik fungerer det'},onboarding:{num:'ONB',label:'Rolleintro'},'participant-profile':{num:'P',label:'Min profil'},admin:{num:'07',label:'Administrasjon'},crm:{num:'CRM',label:'Mini CRM'}
};
...
function persistMainSnapshot(nav){
  try{
    const items=[...nav.querySelectorAll('.nav-item')].filter(navItemVisible).map(el=>({num:el.querySelector('.nav-num')?.textContent?.trim()||'',label:el.querySelector('b')?.textContent?.trim()||'',href:navItemHref(el),sub:el.classList.contains('nav-subitem'),badges:snapshotBadges(el)})).filter(x=>x.label&&x.href);
...
function hrefPage(href){
  if(!href)return'';try{const u=new URL(href,location.href),parts=u.pathname.split('/').filter(Boolean),last=parts.pop()||'index.html';if(!last.includes('.'))return'index';return last.replace('.html','')}catch{return''}
}
function itemMatchesPage(item,currentPage,meta){if(!item)return false;if(hrefPage(item.href)===currentPage)return true;return !!meta&&item.label===meta.label}
function standaloneFromSnapshot(nav,meta){
  ...
  if(!found&&meta){const current=makeItem({num:meta.num,label:meta.label,active:true});const helpIndex=items.findIndex(el=>el.querySelector('b')?.textContent?.trim()==='Hjelp & SOS');items.splice(helpIndex>=0?helpIndex:items.length,0,current)}
...
function normalizeStandalone(){
  ...
  if(page==='guide')items.push(makeItem({num:'00',label:'Slik fungerer det',active:true}));else items.push(makeItem({href:'./guide.html',num:'00',label:'Slik fungerer det'}));
  if(page==='admin'){items.push(makeItem({num:'07',label:'Administrasjon',active:true}));items.push(makeItem({href:'./audit.html',num:'07A',label:'Revisjon',sub:true}))}
  else if(page==='audit'){items.push(makeItem({href:'./admin.html',num:'07',label:'Administrasjon'}));items.push(makeItem({num:'07A',label:'Revisjon',active:true,sub:true}))}
  else if(!['guide','sos'].includes(page))items.push(makeItem({num:meta.num,label:meta.label,active:true}));
...
function applyHashView(nav){const raw=location.hash.slice(1);if(!raw||!/^[a-z-]+$/.test(raw))return;const target=nav.querySelector(`.nav-item[data-view="${raw}"]`);if(!target||!navItemVisible(target))return;target.click();history.replaceState(null,'',location.pathname+location.search)}
...
function normalizeMain(){
  ...
  const order=['overview','participants','#intakeNav','#ownersNav','tasks','checkin','forms','#pilotOpsNav','#adminLink','#crmNav','#guideNav','#sosNav'];order.map(key=>mainNode(nav,key)).filter(Boolean).forEach(el=>nav.appendChild(el));
  nav.dataset.navigationIa=NAV_IA_VERSION;persistMainSnapshot(nav);if(!nav.dataset.snapshotBound){nav.dataset.snapshotBound='1';nav.addEventListener('click',()=>persistMainSnapshot(nav),{capture:true})}

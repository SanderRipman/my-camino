...
assert(mobile.includes("MOBILE_UX_VERSION='2026-09-05b'"),'Shared portal shell must cache-bust the current mobile polish layer.');
assert(mobile.includes("WORKDAY_CHROME_VERSION='2026-09-05b'")&&mobile.includes('app-workday-chrome.js')&&mobile.includes('workday-mobile.css'),'Shared portal shell must load the current workday chrome layer.');
assert(mobile.includes("NAVIGATION_IA_VERSION='2026-09-05b'"),'Shared portal shell must cache-bust the current navigation IA layer.');
...
assert(navIa.includes("NAV_SNAPSHOT_KEY='aidme:navigation-snapshot:v2'")&&navIa.includes('persistMainSnapshot(nav)')&&navIa.includes('standaloneFromSnapshot(nav,meta)'),'All standalone workspaces must restore the actual role-aware main navigation from a safe same-origin session snapshot.');

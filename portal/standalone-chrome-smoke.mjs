...
assert(mobile.includes("NAVIGATION_IA_VERSION='2026-09-06b'"),'Shared portal shell loader remains compatible while navigation IA self-invalidates via v7.');
...
assert(mobile.includes("'aidme:navigation-snapshot:v5'")&&mobile.includes('removeItem(key)'),'Legacy v5 navigation snapshot must be discarded.');
...
assert(guide.includes('app-mobile.js?v=20260906b'),'Program guide must load the common mobile/navigation shell.');
...
assert(accessState.includes('app-phase-workspace.js?v=20260909c'),'Portal must load the consolidated phase workspace.');
assert(phaseWorkspace.includes("PHASE_WORKSPACE_VERSION='2026-09-09c'")&&phaseWorkspace.includes('function needsClarification(t)'),'Phase workspace must own semantic clarification presentation.');
assert(phaseWorkspace.includes("function demoOrigin()")&&phaseWorkspace.includes("if(!demoOrigin()||!staff()"),'Demo aggregate continuity must stay hostname-gated and unavailable on my.aidme.no.');
assert(phaseWorkspace.includes("selectedPhase==='ALL'")&&phaseWorkspace.includes("setMetric('#metricYellow',clar)"),'General clarification KPI must use semantic clarification count.');
assert(!phaseWorkspace.includes("severity(t)==='YELLOW'"),'Clarification must not collapse to yellow severity.');
assert(!/client\.from|service_role|secret_key/i.test(phaseWorkspace),'Shared phase workspace must not create a direct database or privileged-key path.');

console.log('Standalone/navigation IA, v7 snapshot, semantic phase presentation, prefetch and lightweight owner-tool smoke: OK');

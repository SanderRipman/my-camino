...
function renderPilotEvaluationEntry(){
  ...
  const links=visible.map(p=>`<a class="gate-link" href="./form-runner.html?key=pilot_evaluation&pilot=${encodeURIComponent(p.id)}"><strong>${escapeHtml(p.name||'Pilot')}</strong><small>Aggregert evaluering${p.status?` · ${escapeHtml(p.status)}`:''} · Åpne →</small></a>`).join('');

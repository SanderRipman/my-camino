...
assert(command.includes('canonicalizeVidaOwner'),'Form command must canonicalize VIDA owner server-side');
assert(command.includes("fd?.key!=='vida_plan'"),'Canonicalization must be limited to VIDA plan');
assert(command.includes('vida_owner:profile.full_name'),'Saved VIDA owner must come from canonical active responsibility');

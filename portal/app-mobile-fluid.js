...
const MOBILE_FLUID_VERSION='2026-09-13d';
...
let gesture=null,suppressClickUntil=0,navGesture=null,navSuppressClickUntil=0;
...
function installNavGesture(){const n=nav(),surface=n?.closest('.sidebar')||n;if(!n||!surface||surface.dataset.aidmeFluidNavTouch==='1')return;surface.dataset.aidmeFluidNavTouch='1';
 surface.addEventListener('touchstart',event=>{navGesture=null;if(!mobile()||event.touches?.length!==1)return;const t=event.touches[0];navGesture={x:t.clientX,y:t.clientY,left:n.scrollLeft,lastX:t.clientX,lastT:performance.now(),horizontal:false,moved:false,velocity:0}},{capture:true,passive:true});
 surface.addEventListener('touchmove',event=>{const g=navGesture;if(!g||event.touches?.length!==1)return;const t=event.touches[0],dx=t.clientX-g.x,dy=t.clientY-g.y;if(!g.horizontal&&Math.abs(dx)>=LOCK_AT&&Math.abs(dx)>Math.abs(dy)*AXIS_RATIO)g.horizontal=true;if(!g.horizontal)return;if(event.cancelable)event.preventDefault();event.stopPropagation();const now=performance.now(),dt=Math.max(1,now-g.lastT),instant=(t.clientX-g.lastX)/dt;g.velocity=g.velocity*.55+instant*.45;g.lastX=t.clientX;g.lastT=now;g.moved=Math.abs(dx)>=10;n.scrollLeft=g.left-dx;syncMarker(false)},{capture:true,passive:false});
 surface.addEventListener('touchend',event=>{const g=navGesture;navGesture=null;if(!g||event.changedTouches?.length!==1)return;const t=event.changedTouches[0],dx=t.clientX-g.x,dy=t.clientY-g.y,axis=Math.abs(dx)>Math.abs(dy)*AXIS_RATIO;if(!g.horizontal||!g.moved||!axis)return;navSuppressClickUntil=performance.now()+500;const max=Math.max(0,n.scrollWidth-n.clientWidth),momentum=clamp(g.velocity,-1.05,1.05)*110,target=clamp(n.scrollLeft-momentum,0,max);n.scrollTo({left:target,behavior:reducedMotion()?'auto':'smooth'})},{capture:true,passive:true});
 surface.addEventListener('touchcancel',()=>{navGesture=null},{capture:true,passive:true});
 surface.addEventListener('click',event=>{if(performance.now()>navSuppressClickUntil)return;event.preventDefault();event.stopPropagation();event.stopImmediatePropagation()},{capture:true});
 n.addEventListener('scroll',()=>syncMarker(false),{passive:true})}

...
assert(access.includes('app-mobile-fluid.js?v=20260917c'),'Portal must load the current unified mobile gesture owner with a fresh cache key.');
...
assert(fluid.includes("MOBILE_FLUID_VERSION='2026-09-17c'")&&fluid.includes('VELOCITY_COMMIT=.34'),'Unified mobile flow must be versioned and velocity-aware.');
...
assert(fluid.includes("style.display!=='none'")&&fluid.includes('Number(rect.width)>0')&&fluid.includes('Number(rect.height)>0'),'Swipe adjacency must use tabs that are physically rendered, so a visible Innsjekk cannot be skipped by stale helper classes.');
assert(!fluid.includes('prepareAdjacent')&&!fluid.includes('aidme-flow-preview')&&!fluid.includes('nextView.style.transform'),'Same-page swipe must not keep a detached destination-page preview that can blink between screens.');
assert(fluid.includes('function commitContent(g)')&&fluid.includes('next.item.click()'),'One committed swipe must activate exactly one adjacent canonical navigation item.');
...
console.log('Early-UAT polish 2026-09-17 canonical swipe stability invariants OK');
...
async function loadSubmissions(){
 if(!currentVersion)return;const participantId=$('#participantSelect').value||null,pilotId=selectedPilot()?.id||null;
 const meta='id,participant_id,pilot_id,form_version_id,submitted_by,status,submitted_at,created_at,updated_at';
 let q=client.from('form_submissions').select(meta).eq('form_version_id',currentVersion.id).order('created_at',{ascending:false}).limit(20);
 q=participantId?q.eq('participant_id',participantId):q.is('participant_id',null);q=pilotId?q.eq('pilot_id',pilotId):q.is('pilot_id',null);
 const {data,error}=await q;if(error){$('#submissionList').innerHTML='<p>Ingen tilgjengelig historikk.</p>';return}
 const rows=data||[];currentDraft=rows.find(r=>r.status==='DRAFT'&&r.submitted_by===session.user.id)||null;
 $('#submissionList').innerHTML=rows.length?rows.map(r=>`<div class="submission-row"><div><b>${r.status==='SUBMITTED'?'Fullført':'Utkast'}</b><small>${new Intl.DateTimeFormat('nb-NO',{dateStyle:'medium',timeStyle:'short'}).format(new Date(r.submitted_at||r.updated_at||r.created_at))}</small></div><span class="pill">v${currentVersion.version}</span></div>`).join(''):'<p>Ingen registreringer for denne konteksten ennå.</p>';

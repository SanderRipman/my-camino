...
async function install(){try{const {data:{session}}=await onboardingClient.auth.getSession();if(!session)return;const {data:grants,error:ge}=await onboardingClient.from('role_grants').select('id,revoked_at,valid_from,valid_until').eq('user_id',session.user.id);if(ge)return;const now=new Date(),active=(grants||[]).filter(g=>!g.revoked_at&&(!g.valid_from||new Date(g.valid_from)<=now)&&(!g.valid_until||new Date(g.valid_until)>now));if(!active.length)return;const nav=addNav(document.querySelector('#mainNav'));addMenu(document.querySelector('#userMenu'));const {count,error}=await onboardingClient.from('role_onboarding_items').select('id',{head:true,count:'exact'}).eq('user_id',session.user.id).neq('status','DONE');if(error)return;const n=count||0;if(nav&&n){const badge=nav.querySelector('#badgeOnboarding');badge.innerHTML=`<span class="nav-count yellow">${Math.min(n,99)}</span>`;nav.setAttribute('aria-label',`${n} uferdige rolleintroduksjonssteg`)}}
catch(e){console.warn('[AidMe VIDA onboarding nav]',e)}}

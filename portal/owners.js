...
async function initCore(){
  ...
  if(!ok){$('#blocked').classList.remove('hidden');$('#work').classList.add('hidden');return}
  ...
  participants=data||[];
  if(error||!participants.length){$('#blocked').classList.remove('hidden');$('#work').classList.add('hidden');return}
  ...
  if(!probe){$('#blocked').classList.remove('hidden');$('#work').classList.add('hidden');return}
  $('#blocked').classList.add('hidden');$('#work').classList.remove('hidden');
}
function init(){
  if(initPromise)return initPromise;
  initPromise=initCore().catch(()=>{$('#blocked')?.classList.remove('hidden')}).finally(()=>{initPromise=null});
  return initPromise;
}
...
async function loadContext(){
  ...
  const {data,error}=await cmd('LIST_CONTEXT',{participantId});
...
$('#participantSelect').addEventListener('change',loadContext);
...
client.auth.onAuthStateChange(event=>{
  if(['SIGNED_IN','TOKEN_REFRESHED','USER_UPDATED','MFA_CHALLENGE_VERIFIED'].includes(event))setTimeout(init,0);
});
init();

...
const taskWorkflowContextPath=path.join(dir,'app-task-workflow-context.js');
...
const authReturnPath=path.join(dir,'app-auth-return.js');
...
const participantPhasePath=path.join(dir,'app-participant-phase.js');
...
const taskWorkflowContext=fs.readFileSync(taskWorkflowContextPath,'utf8');
...
const authReturn=fs.readFileSync(authReturnPath,'utf8');
...
const participantPhase=fs.readFileSync(participantPhasePath,'utf8');
...
code += '\n\n/* Task workflow metadata is rehydrated read-only so workflow-specific handoffs can route on canonical task keys. */\n'+taskWorkflowContext+'\n';
...
code += '\n\n/* Protected auth return preserves a requested deep link through login/AAL2 without weakening the gate. */\n'+authReturn+'\n';
...
code += '\n\n/* Staff participant list gets a single phase filter and one right-aligned phase badge; visibility still comes from existing grants/RLS. */\n'+participantPhase+'\n';

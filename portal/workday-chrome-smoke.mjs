...
assert(navScroll.includes("MOBILE_NAV_SCROLL_VERSION='2026-09-13c'"),'Mobile nav drag must be cache-busted after the clickable-surface fix.');
assert(navScroll.includes('.sidebar,.sidebar nav,.sidebar nav .nav-item{touch-action:pan-y!important}'),'Vertical page scrolling must remain native while the nav script owns horizontal drag.');
...
assert(!navScroll.includes('insideNav:nav.contains(event.target)'),'Clickable nav items must not be delegated to inconsistent native horizontal handling.');
assert(navScroll.includes('if(!start.horizontal&&Math.abs(dx)>=LOCK_AT')&&navScroll.includes('if(event.cancelable)event.preventDefault()')&&navScroll.includes('nav.scrollLeft=start.left-dx'),'Horizontal drag must lock explicitly and drive the same scroll path from clickable and quiet surfaces.');
assert(navScroll.includes('{passive:false,capture:true}'),'Horizontal touchmove owner must be allowed to prevent default only after axis lock.');

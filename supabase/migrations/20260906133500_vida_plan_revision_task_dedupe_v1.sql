-- VIDA living-plan revisions must not reopen completed milestone tasks.
-- Initial plan submission may create missing 72h/14d/30d/90d tasks once.
-- Later immutable revisions reuse the same milestone sequence.
...
begin
  ...
  if new.participant_id is not null and k in ('interest_referral','via_roadmap','participant_agreement') then
    ...
    -- Unlike generic task enqueueing, VIDA milestone history is intentionally
    -- deduplicated across DONE/CANCELLED as well as active states. A later plan
    -- revision must never infer a new milestone cycle. A future explicit new VÍA
    -- owns creation of any genuinely new cycle.
    if not exists (
      select 1 from public.tasks t
      where t.organization_id = new.organization_id
        and t.participant_id = new.participant_id
        and t.pilot_id is not distinct from new.pilot_id
        and t.workflow_key = 'vida_72h'
    ) then

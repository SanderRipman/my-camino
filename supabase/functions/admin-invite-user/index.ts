...
const allowedOrigins = new Set([
  'https://my.aidme.no',
  'https://main--mycamino.netlify.app',
  'http://localhost:8888',
  'http://localhost:3000',
])
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!publishable || !secret) throw new Error('Missing Supabase function keys')
    ...
    const claims = decodeClaims(token) as Record<string, unknown>
    if (claims.aal !== 'aal2') return new Response(JSON.stringify({ error: 'MFA_REQUIRED' }), { status: 403, headers })

    const now = new Date().toISOString()
    const { data: grants, error: grantError } = await admin
      .from('role_grants')
      .select('id,role_code,valid_from,valid_until,revoked_at')
    ...
    return new Response(JSON.stringify({ ok: true, userId: data.user?.id ?? null, email }), { status: 200, headers })
  } catch (error) {
    console.error(error)

...
Deno.serve(async(req:Request)=>{const headers=cors(req);if(req.method==='OPTIONS')return new Response('ok',{headers});if(req.method!=='POST')return new Response(JSON.stringify({error:'METHOD_NOT_ALLOWED'}),{status:405,headers});try{
 ...
 if(action==='START_SER'){
   ...
   if(oldStage!=='SER')return new Response(JSON.stringify({error:'VIDA_REQUIRES_SER',stage:oldStage}),{status:409,headers});if(!await capability(admin,userData.user.id,p.organization_id,participantId,pilotId,['manage_tasks','edit_ser']))return new Response(JSON.stringify({error:'FORBIDDEN'}),{status:403,headers})

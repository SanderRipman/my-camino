...
function grantScopeMatches(g:any,pilotId:string){return !g.revoked_at&&!g.participant_id&&(!g.pilot_id||String(g.pilot_id)===pilotId)}
function currentRequesterGrant(g:any,pilotId:string){const now=new Date();return grantScopeMatches(g,pilotId)&&!!g.valid_from&&new Date(g.valid_from)<=now&&(!g.valid_until||new Date(g.valid_until)>now)}
function currentCandidateGrant(g:any,pilotId:string){const now=new Date();return grantScopeMatches(g,pilotId)&&(!g.valid_from||new Date(g.valid_from)<=now)&&(!g.valid_until||new Date(g.valid_until)>now)}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const requestRoles=[...new Set((requestGrants??[]).filter((g:any)=>currentRequesterGrant(g,pilotId)).map((g:any)=>String(g.role_code)))];if(!requestRoles.length)return new Response(JSON.stringify({error:'FORBIDDEN'}),{status:403,headers})
  ...
  const validCandidateGrants=(candidateGrants??[]).filter((g:any)=>currentCandidateGrant(g,pilotId));const candidateRoles=[...new Set(validCandidateGrants.map((g:any)=>String(g.role_code)))];if(!candidateRoles.length)return new Response(JSON.stringify({staff:[]}),{status:200,headers})
